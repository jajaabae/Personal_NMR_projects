...
def getRoesyCorrelations(hName, NewAtomData, correlations):
#def getTocsyCorrelations(hName, NewAtomData, correlations):
    returnString = ''
    HNumberedName = getHNumberedNameFromHName(hName, NewAtomData)
    corrList = []
...
def getCHText(inLine, NewAtomData, correlations):
    ...
    line+='\t'
    line+=getRoesyCorrelations(inLine[2], NewAtomData, correlations)
    return line
...
def replaceCorrsWithFullNamesOrPpmValues(corrList, NewAtomData):
    retList = []
    ...
    if c.B == True:
        if not c.convertKvownNamesToNumbers: # replace with full names
            ...
            for i in corrList:
                ...
                retList.append( ppmValue(numberedName, NewAtomData))
...
def ppmValue(hName, NewAtomData):
    returnString = ''
    for d in NewAtomData:
...
def fun_exportSciTables(Carbons, Hydrogens, NewAtomData, correlations, convertKvownNames, convertKvownNamesToNumbers, onlyPureRoesyCorrelations):
