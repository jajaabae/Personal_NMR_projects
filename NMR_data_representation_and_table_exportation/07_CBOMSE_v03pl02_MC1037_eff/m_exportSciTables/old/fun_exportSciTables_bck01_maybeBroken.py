...
def getCorrelationsFromNumberedName(HNumberedName, corType, correlations):
    cosyCorrelations = []
    #correlationsCOPY = list(correlations)
    #print type(correlationsCOPY)
    #for cor in correlationsCOPY:
    for cor in correlations:
        #print type(cor)
        corCopy = list(cor)
        if corType in cor and HNumberedName in cor:
            ...
            #print cor
            #if not len(cosyCorrelations)==0:
            #    cosyCorrelations+=', '
            cosyCorrelations+=[corCopy[0]]
            #print cosyCorrelations
            #pass
...
def getCHText(inLine, NewAtomData, correlations):
    line = ''
    line += inLine[0]
    line+='\t'
    if not inLine[1] == '':
    ...
    line+='\t'
    line+=getCNumberedNameFromCName(inLine[2], NewAtomData)
    line+='\t'
    line+=getCText(inLine[1], NewAtomData)    

    line+='\t'
    line+=getHNumberedNameFromHName(inLine[1], NewAtomData)
    line+='\t'
    line+=getHText(inLine[2], NewAtomData)
    
    line+='\t'
    line+=getCosyCorrelations(inLine[2], NewAtomData, correlations)
    line+='\t'
    line+=getTocsyCorrelations(inLine[2], NewAtomData, correlations)
    line+='\t'
    line+=getHsqcCorrelations(inLine[2], NewAtomData, correlations)
    line+='\t'
    line+=getHmbcCorrelations(inLine[2], NewAtomData, correlations)
    
    return line

def printInTableFormat(Carbons, Hydrogens, NewAtomData, correlations, tableFormat):
    lines = []
    startLine = 'aa	name	c_num	c	h_num	h	cosy	tocsy	hsqc	hmbc'
    lines.append(startLine)
    for line in tableFormat:
        lines.append(getCHText(line, NewAtomData, correlations))
...
def getPrintTableFormat():
    table = [
            ...
            ['',	'Adda5 7',	'Adda5 7'],
            ['',	'Adda5 8',	'Adda5 8'],
            ['',	'Adda5 8-Me',	'Adda5 8-Me'],
            ['',	'Adda5 9',	'Adda5 9'],
            ['',	'Adda5 9-OMe',	'Adda5 9-OMe'],
            ['',	'Adda5 10',	'Adda5 10a'],
            ['',	'',	'Adda5 10b'],
            ['',	'Adda5 11',	''],
            ['',	'Adda5 12/16',	'Adda5 12/16'],
            ['',	'Adda5 13/15',	'Adda5 13/13'],
            ['',	'Adda5 14',	'Adda5 14'],
            ['Glu6',	'Glu6 1',	''],
            ['',	'Glu6 2',	'Glu6 2'],
            ['',	'',	'Glu6 2-NH'],
...
def fun_exportSciTables(Carbons, Hydrogens, NewAtomData, correlations):
    printInTableFormat(Carbons, Hydrogens, NewAtomData, correlations, getPrintTableFormat())
