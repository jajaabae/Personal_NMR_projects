...
def getCosyCorrelations(hName, NewAtomData, correlations):
    ...
    corrList = replaceCorrsWithFullNames(corrList, NewAtomData)
...
def printInTableFormat(Carbons, Hydrogens, NewAtomData, correlations, tableFormat):
    lines = []
    startLine = 'Amino acid	Atom name	c_num	C	h_num	H	COSY	TOCSY	HSQC	HMBC'
...
def replaceCorrsWithFullNames(corrList, NewAtomData):
    retList = []
...
def fun_exportSciTables(Carbons, Hydrogens, NewAtomData, correlations, convertKvownNames, convertKvownNamesToNumbers):
    c = ReplaceNamesBooleanContainer()
    #c.setBool(False)
    #c.setBool(True)
