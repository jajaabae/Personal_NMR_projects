...
def getCHText(inLine, NewAtomData, correlations, md_object):
    line = ''
    line += inLine[0]
    line+='\t'
    if not inLine[1] == '':
    ...
    line+='\t'
    line+=getRoesyCorrelations(inLine[2], NewAtomData, correlations)#pure_ROESY
...
def printInTableFormat(Carbons, Hydrogens, NewAtomData, correlations, tableFormat, md_object):
    lines = []
    #startLine = 'Amino acid	Atom name	c_num	C	h_num	H	COSY	TOCSY	HSQC	HMBC	ROESY'
    startLine = 'Amino acid	Atom name	C	HSQC_+/-	H	Multip.	N(H)	COSY	TOCSY	HSQC	HMBC	ROESY	mult_str_for_copy	carbon_str'
    lines.append(startLine)
    for line in tableFormat:
        lines.append(getCHText(line, NewAtomData, correlations, md_object))
