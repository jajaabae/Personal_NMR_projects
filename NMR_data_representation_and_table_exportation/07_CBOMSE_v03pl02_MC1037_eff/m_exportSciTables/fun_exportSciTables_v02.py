...
def formatDualWayCorrelationList(corrList, force_one_way=False):
    #formatDualOrSingleWayCorrelationList
    ...
    def formatDualWayCorrelationList_sub(corrList):
        corrString = ''
        #from fun_splitListIntoSingleAndMultipleOccurences import splitListIntoSingleAndMultipleOccurences
        i_fsf('m_exportSciTables.splitListIntoSingleAndMultipleOccurences')
        singles, multiple = splitListIntoSingleAndMultipleOccurences(corrList)
    ...
    singleWayCor = True #remove 's' for single correlations
    if singleWayCor or force_one_way:
        return formatOneWayCorrelationList(corrList)
    else:
        return formatDualWayCorrelationList_sub(corrList)
...
def getHsqcCorrelations(hName, NewAtomData, correlations):
    ...
    returnString += formatDualWayCorrelationList(corrList, force_one_way=True)
    if returnString =='':
        #returnString='-'
        returnString = defaultStringContainer.defaultString
    return returnString
...
def getCHText_dict(tableFormat, NewAtomData, correlations, md_object, includeExtraColumns):
    dic = {
...
'in_ROESY':['in_ROESY'],
'all_ROESY':['all_ROESY'],
'mult_str_for_copy':['mult_str_for_copy'],
'carbon_str':['carbon_str'],
'C-name':['C-name'],
'H-name':['H-name'],
    }

    for inLine in tableFormat:
        #lines.append(getCHText(tableFormat, NewAtomData, correlations, md_object, includeExtraColumns))
        
        line = ''
        #line += inLine[0]
        dic['Amino acid'].append(inLine[0])
        #line+='\t'
        
        if not inLine[1] == '':
            #line += "'"+inLine[1].split(' ')[1]
            dic['Atom name'].append( inLine[1].split(' ')[1] )
        elif not inLine[2] == '' and 'NH' in inLine[2]:
            #line += "'"+inLine[2].split(' ')[1]
            dic['Atom name'].append( inLine[2].split(' ')[1] )
        else:
            dic['Atom name'].append( '' )
        ...
        #line+='\t'
        #line+=getCText(inLine[1], NewAtomData)#C
        dic['C'].append( getCText(inLine[1], NewAtomData) )
        ...
        #line += get_data_from_md_object(md_object, inLine[1], 'HSQC_pos_neg')
        ##print get_data_from_md_object(md_object, inLine[2], 'HSQC_pos_neg')
        dic['HSQC_+/-'].append( get_data_from_md_object(md_object, inLine[1], 'HSQC_pos_neg') )
        ...
        #line+='\t'
        #line+=getHText(inLine[2], NewAtomData)#H
        dic['H'].append( getHText(inLine[2], NewAtomData) )
        ...
        #line += get_data_from_md_object(md_object, inLine[2], 'mult_str')
        #print get_data_from_md_object(md_object, inLine[2], 'mult_str'),
        dic['Multip.'].append( get_data_from_md_object(md_object, inLine[2], 'mult_str') )

        #Number of Hydrogens
        #line += '\t'
        #line += get_data_from_md_object(md_object, inLine[2], 'number_of_hydrogens')
        #print get_data_from_md_object(md_object, inLine[2], 'number_of_hydrogens'),
        dic['N(H)'].append( get_data_from_md_object(md_object, inLine[2], 'number_of_hydrogens') )

       
        #line+='\t'
        #line+=getCosyCorrelations(inLine[2], NewAtomData, correlations)#COSY
        dic['COSY'].append( getCosyCorrelations(inLine[2], NewAtomData, correlations) )
        
        #line+='\t'
        #line+=getTocsyCorrelations(inLine[2], NewAtomData, correlations)#TOCSY
        dic['TOCSY'].append( getTocsyCorrelations(inLine[2], NewAtomData, correlations) )
        
        #line+='\t'
        #line+=getHsqcCorrelations(inLine[2], NewAtomData, correlations)#HSQC
        dic['HSQC'].append( getHsqcCorrelations(inLine[2], NewAtomData, correlations) )
        
        #line+='\t'
        #line+=getHmbcCorrelations(inLine[2], NewAtomData, correlations)#HMBC
        dic['HMBC'].append( getHmbcCorrelations(inLine[2], NewAtomData, correlations) )
        
        line+='\t'
        typeOfRoesyCorrelations = 'external'
        #line+=getRoesyCorrelations(inLine[2], NewAtomData, correlations, typeOfRoesyCorrelations)#pure_ROESY
        dic['ex_ROESY'].append( getRoesyCorrelations(inLine[2], NewAtomData, correlations, typeOfRoesyCorrelations) )
        
        line+='\t'
        typeOfRoesyCorrelations = 'internal'
        line+=getRoesyCorrelations(inLine[2], NewAtomData, correlations, typeOfRoesyCorrelations)#internal_ROESY
        dic['in_ROESY'].append( getRoesyCorrelations(inLine[2], NewAtomData, correlations, typeOfRoesyCorrelations) )
        
        #line+='\t'
        typeOfRoesyCorrelations = 'all'
        #line+=getRoesyCorrelations(inLine[2], NewAtomData, correlations, typeOfRoesyCorrelations)#std_ROESY
        dic['all_ROESY'].append( getRoesyCorrelations(inLine[2], NewAtomData, correlations, typeOfRoesyCorrelations) )
        ...
        if includeExtraColumns:
            ...
            if not mult_str_for_copy == getHText(inLine[2], NewAtomData)+' ppm ':
                #line += mult_str_for_copy
                dic['mult_str_for_copy'].append( mult_str_for_copy )
            else:
                dic['mult_str_for_copy'].append( '' )
            ...
            if not carbon_str == ' ppm ()':
                #line+= carbon_str
                dic['carbon_str'].append( carbon_str )
            else:
                dic['carbon_str'].append( '' )
            
            #C-name
            #line+='\t'
            #line+=inLine[1] #C
            dic['C-name'].append( inLine[1] )
            
            #H-name
            #line+='\t'
            #line+=inLine[2] #H
            dic['H-name'].append( inLine[2] )
...
def printInTableFormat(Carbons, Hydrogens, NewAtomData, correlations, tableFormat, md_object): #includeExtraColumns, print_table_in_shell):
    ...
    dic = getCHText_dict(tableFormat, NewAtomData, correlations, md_object, includeExtraColumns)
    #for e in dic:
    #    print len(dic[e]), e#, dic[e]
    #print
    #e = 'HSQC_+/-'
    #print len(dic[e]), e, dic[e]


    f = open('exports/exportedSciTable.tbl', 'w')
    length = len(dic['Amino acid'])
    width = 0
    #print len(dic['Amino acid'])
    ...
    f = open('exports/exportedSciTable_TOCSY_ROESY.tbl', 'w')
    length = len(dic['Amino acid'])
    width = 0
    #print len(dic['Amino acid'])
    for i in range(length):
    ...
    print '\nTable size:', len(lines[0].split('\t')), 'x', len(lines), '\n'
    #"""
...
def fun_exportSciTables_v02(Carbons, Hydrogens, NewAtomData, correlations, convertKvownNames, convertKvownNamesToNumbers, onlyPureRoesyCorrelations, md_object):
